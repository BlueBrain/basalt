...
void register_circuit_payloads_bindings(py::module& basalt) {
    ...
    py::class_<circuit::float_point_t>(m, "Point")
        ...
        .def("__iter__",
             ...
             py::keep_alive<0, 1>())
        .def("__getitem__",
             [](const circuit::float_point_t& p, int index) {
        ...
        .def_property("x",
                      [](const circuit::float_point_t& p) { return p[0]; },
                      [](circuit::float_point_t& p, circuit::float_point_t::value_type value) {
                          p[0] = value;
                      })

        .def_property("y",
                      [](const circuit::float_point_t& p) { return p[1]; },
                      [](circuit::float_point_t& p, circuit::float_point_t::value_type value) {
                          p[1] = value;
                      })

        .def_property("z",
                      [](const circuit::float_point_t& p) { return p[2]; },
                      [](circuit::float_point_t& p, circuit::float_point_t::value_type value) {
                          p[2] = value;
                      });
    ...
    py::class_<circuit::neuron_t>(m, "Neuron")
        .def(py::init<>())
        .def(py::init(&circuit::neuron_t::create),
             "gid"_a,
    ...
    py::class_<circuit::synapse_t>(m, "Synapse")
        .def(py::init<>())
        .def(py::init(&circuit::synapse_t::create),
             "pre_gid"_a = 0,
             "post_gid"_a = 0,
             "nrn_idx"_a = 0,
             "astro_idx"_a = py::array_t<uint32_t>(0),
    ...
    py::class_<circuit::astrocyte_t>(m, "Astrocyte")
        .def(py::init<>())
        .def(py::init(&circuit::astrocyte_t::create),
             "astrocyte_id"_a = 0,
             "microdomain_id"_a = 0,
    ...
    py::class_<circuit::microdomain_t>(m, "MicroDomain")
        .def(py::init<>())
        .def(py::init(&circuit::microdomain_t::create),
             "microdomain_id"_a = 0,
             "astrocyte_id"_a = 0,
             "neighbors"_a = py::array_t<uint32_t>(0),
             "vertex_coordinates"_a = py::array_t<float>(0),
             "triangles"_a = py::array_t<uint32_t>(0),
             "centroid"_a = circuit::float_point_t({{0., 0., 0.}}),
             "area"_a = 0.,
             "volume"_a = 0.,
             "mesh_filename"_a = "",
             "neurons_idx"_a = py::array_t<uint32_t>(0),
    ...
    py::class_<circuit::segment_t>(m, "Segment")
        .def(py::init<>())
        .def(py::init<>(&circuit::segment_t::create),
             "section_id"_a = 0,
             "segment_id"_a = 0,
             "type"_a = 0,
             "x1"_a = 0.,
