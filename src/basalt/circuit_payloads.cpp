...
neuron_t::neuron_t(uint32_t gid, int_vector_t& astro_idx, int_vector_t& syn_idx)
    : gid(gid), astro_idx(std::move(astro_idx)), syn_idx(std::move(syn_idx)) {}
...
synapse_t::synapse_t(uint32_t pre_gid, uint32_t post_gid, uint32_t nrn_idx,
    ...
    : pre_gid(pre_gid), post_gid(post_gid), nrn_idx(nrn_idx),
      astro_idx(std::move(astro_idx)), is_excitatory(is_excitatory),
      pre(std::move(pre)), post(std::move(post)), mesh_filename(mesh_filename),
      skeleton_filename(skeleton_filename), psd_area(psd_area) {}
...
microdomain_t::microdomain_t(uint32_t microdomain_id, uint32_t astrocyte_id,
                             ...
                             const float_point_t& centroid, double area,
                             double volume, const std::string& mesh_filename,
                             int_vector_t& neurons_idx,
                             int_vector_t& synapses_idx)
    : microdomain_id(microdomain_id), astrocyte_id(astrocyte_id),
      neighbors(std::move(neighbors)),
      vertex_coordinates(std::move(vertex_coordinates)),
      triangles(std::move(triangles)), centroid(centroid), area(area),
      volume(volume), mesh_filename(mesh_filename),
      neurons_idx(std::move(neurons_idx)),
      synapses_idx(std::move(synapses_idx)) {}
...
segment_t::segment_t(uint32_t section_id, uint32_t segment_id, uint8_t type,
                     float x1, float y1, float z1, float r1, float x2, float y2,
                     float z2, float r2)
    : section_id(section_id), segment_id(segment_id), type(type), x1(x1),
      y1(y1), z1(z1), r1(r1), x2(x2), y2(y2), z2(z2), r2(r2) {}
...
std::unique_ptr<segment_t> segment_t::create(uint32_t section_id,
                                             ...
                                             float z2, float r2) {
    return std::unique_ptr<segment_t>(new segment_t(
...
edge_astrocyte_segment_t::edge_astrocyte_segment_t(
    const float_point_t& astrocyte, const float_point_t& vasculature)
    : astrocyte(astrocyte), vasculature(vasculature) {}
